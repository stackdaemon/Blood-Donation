...
const AllDonationRequests = () => {
  ...
  // Edit click handlers (Admin only)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">All Blood Donation Requests</h2>
        <p className="text-slate-500 text-sm">
          {isAdmin ? 'Complete CRUD management of all platform donation requests.' : 'Review and update status on platform requests.'}
        </p>
      </div>
      ...
      <div className="bg-white border border-slate-200/60 rounded-3xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/40">
          <h3 className="font-bold text-slate-800 text-sm">Donation Request Feed Records</h3>
        </div>
        ...
        {loading ? (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-500 font-bold border-b border-slate-100 uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4">Hospital / Address</th>
                ...
                {requests.map((req) => (
                  <tr key={req._id} className="hover:bg-slate-50/50">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5 font-mono">{req.requesterEmail}</div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="py-20 text-center text-slate-400 space-y-2">
            ...
            <p className="text-xs font-semibold">No donation requests logged in the database.</p>
          </div>
        )}
      </div>

      {/* Edit Donation Request Modal Drawer (Admin only) */}
      {isAdmin && editModalOpen && editingRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-in fade-in duration-200">
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-lg w-full relative z-10 overflow-hidden animate-in zoom-in-95 duration-150">
            ...
            <div className="bg-rose-500 py-5 px-6 text-white flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold">Edit Donation Request (Admin Mode)</h3>
